...
Future<void> _handleNotificationNavigation(RemoteMessage msg) async {
  ...
  if (action == 'NEW_REQUEST') {
    ...
    if (uid != null) {
      ...
      // ✅ Admin-controlled face verification check
      final data = doc.data() ?? {};
      final faceVerified =
          data['faceVerified'] ?? true; // default true for older users
      if (!faceVerified) {
        await FirebaseAuth.instance.signOut();

        // ✅ Use navigatorKey for safe, global UI feedback
        ...
        if (currentContext != null) {
          ...
          ScaffoldMessenger.of(currentContext).showSnackBar(
            const SnackBar(
              content: Text(
                'Your face verification has been disabled by admin.',
              ),
      ...
      data: (user) {
        ...
        return userDocAsync.when(
          data: (doc) {
            ...
            // Selfie verification for riders and drivers
            Future.microtask(() async {
              ...
              if (needVerify) {
                final result = await navigatorKey.currentState?.push(
                  MaterialPageRoute(
                    builder: (context) =>
                        const FullScreenCamera(isSelfie: true),
                  ),
                );
                if (result is File) {
                  ...
                  if (verificationResult.isValid) {
                    ...
                    final remaining = await SelfieStorage.incrementAttempt(
                      user.uid,
                    );
                    if (remaining == 0) {
